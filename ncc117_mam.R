...
mam.c <- read.csv(file = "mamdata.csv")
...
## 2067 remarks for small mammals from 2023 - 2024
...
#2. Total number of remarks per year by domain
...
# Show remarks only
mam.p.remarks<- subset(mam.p, !(mam.p$remarks == ""))
## 1073 remarks for small mammals from 2023 - 2024
...
# 3. Graphs to visualize total number of remarks for 2 seasons:
ggplot(n_remarks_per_year.2, aes(x=domainID, y=n, fill = domainID))+
  geom_col(position = "dodge", stat = "identity")
...
# Summarize remarks for trap collection for traps without captures (statuses 1,2,3,6)

mam.nocaps <- mam.c %>% filter(trapStatus=="1 - trap not set"|
                                 trapStatus=="2 - trap disturbed/door closed but empty"|
                                 trapStatus=="3 - trap door open or closed w/ spoor left"|
                                 trapStatus=="6 - trap set and empty") 

# Show remarks only
mam.remarks.nocaps <- subset(mam.nocaps, !(mam.nocaps$remarks == ""))
## 2737 remarks for small mammals from 2023 - 2024

mam.remarks.nocaps <- mam.remarks.nocaps %>%         
  mutate(collectDate = as.Date(collectDate), 
         year = year(collectDate), month = month(collectDate), day = day(collectDate))

### total number of remarks per year:
 n_remarks.nocaps <- mam.remarks.nocaps %>%
   group_by(year) %>%
   summarise(n = n()) %>%
   arrange(desc(n))
## 2024 = 1165, 2023 = 1572)
 
 # 1. Raw data = number of each remark
 # Group  by domain and year, and sort # of remarks from high to low
 n_remarks <- mam.remarks.nocaps %>%
   group_by(domainID, year, remarks) %>%
   summarise(n = n())%>%
   arrange(desc(n))
 View(n_remarks)
 
 write.csv(n_remarks, "C:/Users/mccahill/Documents/neon_r_scripts/NCC_117/mammals/mam.remarks.raw.child.NOcaps.csv", row.names = FALSE)
 
 #2. Total number of remarks per year by domain
 n_remarks_per_year <- mam.remarks.nocaps %>%
   group_by(domainID, year) %>%
   summarise(n = n())
 View(n_remarks_per_year)
 ...
 write.csv(n_remarks_final, "C:/Users/mccahill/Documents/neon_r_scripts/NCC_117/mammals/mam.remarks.n.child.NOcaps.csv", row.names = FALSE)
